...
#include "camera.h"
#include "sphere.h"
#include "material-addmaterials.h"
...
{
    ofstream output;
    output.open("chapter8-addmaterials.ppm");

    int nx = 200, ny = 100 , ns = 100;
    output << "P3\n" << nx << " " << ny << "\n255\n";

    // 新增两个球，原来的两个磨砂，小球左右各放个金属材质的，可以反射出中间小球的样子
    hitable *list[4];   // 建立可命中的物体组，存为链表。抽象父类指向继承该父类的球体子类。
    list[0] = new sphere(vec3(0, 0, -1), 0.5, new lambertian(vec3(0.8, 0.3, 0.3)));      // 中间球
    list[1] = new sphere(vec3(0, -100.5, -1), 100, new lambertian(vec3(0.8, 0.8, 0.0))); // 下面的大球
    list[2] = new sphere(vec3(1, 0,-1), 0.5, new metal(vec3(0.8,0.6,0.2)));  // 右球
    list[3] = new sphere(vec3(-1,0,-1), 0.5, new metal(vec3(0.8,0.8,0.8)));  // 左球
